...
function NavbarPopUp(props: NavbarPopUpProps) {
  ...
  const nav_link_class_active = nav_link_class
    .replace("text-black", " text-red-primary underline")
    .replace("text-2xl", "text-6xl")
    .replace(" hover:text-4xl", "");
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 h-screen">
        {props.menuItems.map((item, index) => (
          <li className="flex items-center justify-center" key={index}>
            <Link href={item.href}>
              <a className={props.route.pathname === item.href ? nav_link_class_active : nav_link_class}>{item.name}</a>
            </Link>
          </li>
        ))}
      </div>
